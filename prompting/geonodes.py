...
code_editing_task = TaskSpec(name="Blender code editing for geometry nodes",
        description="You're an experienced python programmer within the Blender environment. You must change the code to make the desired procedural 3D model.",
...
pruning_task = TaskSpec(name="Evaluate which procedural 3D model is more visually similar to some target model.",
        description="You're an experienced Blender 3D artist with a keen eye for materials and geometry. You must compare two different models and indicate which one is more similar to the target model.",
...
def craft_eval_question(
    ...
    if use_vision: 
        if target_image is None:
            ...
            prompt2 = ["Below, I show two different models. Which one is visually more similar to the desired model described? The one on the left or right?",]
                
        else:
            prompt1 = [("Here is the target model rendering:" if target_description is None else f"Here's the target model rendering of {target_description}:"),
                target_image]
            prompt2 = ["Below, I show two different models. Which one is visually more similar to the target model rendering? The one on the left or right?",]
        ...
        question_to_critic = Question([f"Our desired target model can be described by: {target_description}.",
            "Imagine I'm showing you two Blender python scripts for 3D models, and they're side by side. Which one has the highest chance of producing the desired target model in Blender? The one on the left or right?",
            f"Code on the LEFT:\n```python\n{left_code}\n```",
            f"Code on the RIGHT:\n```python\n{right_code}\n```",
...
```
This creates a procedural 3D model and produces the rendering on the left below:
"""
        else:
            if target_image is None:
...
{blender_init_code_str}
```
This creates a procedural 3D model and produces the rendering on the left below. However, the desired model we'd like to create is the target model {target_description}
"""
            else:
                part1 = f"""
The following Blender code was used to produce a procedural 3D model:
...
{blender_init_code_str}
```
This creates a procedural 3D model and produces the rendering on the left below. The model on the right is the target model {target_description}:
"""
                
        if target_image is None:
            # No target image, so use target_description
            part2 = f"""
The desired 3D model is described by {target_description}.
...
"""
            ...
            # With target image
            part2 = f"""
The desired 3D model is shown in the image on the right. 
Answer the following questions:
...
{blender_init_code_str}
```
However, the desired 3D model we'd like to create is the target model {target_description}
""" 
        part2 = f"""
The desired 3D model is described by {target_description}.
Answer the following questions:
...
{blender_init_code_str}
```
The final code creates a procedural 3D model and produces the rendering on the left below:
"""
        else:
            if target_image is None:
                part1 = f"""
...
{blender_init_code_str}
```
The final code creates a procedural 3D model and produces the rendering below. However, the desired 3D model we'd like to create is the target model {target_description}
"""
            else:
                part1 = f"""
The following Blender code was used to produce a procedural 3D model:
...
{blender_init_code_str}
```
The final code creates a procedural 3D model and produces the rendering on the left below. The 3D model on the right is the target model {target_description}:
"""

        if target_image is None: 
            part2 = f"""
The desired procedural 3D model is previously described. Imagine that desired target model. Please describe the difference between the 3D model shown and the desired target model, and edit the code above to reflect this desired change.
"""

        else:
            part2 = f"""
The desired procedural 3D model is shown in the image on the right. Please describe the difference between the two 3D models, and edit the code above to reflect this desired change. 
"""
    else:
        part1 = f"""
The following Blender code was used to produce a procedural 3D model:
...
"""
        part2 = f"""
The desired 3D model is previously described. Imagine that desired target model. Please describe the difference between the model that would be generated by the code and the desired target model, and edit the code above to reflect this desired change.
"""
